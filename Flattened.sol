...
abstract contract ClaimTokenPermit is EIP712 {
    mapping(address => uint256) private _nonces;
    address private immutable owner;
    bytes32 private constant _CLAIM_TOKEN =
        keccak256(
            "claimToken(uint256 uuid, uint256 amount, address userAddress, uint256 nonce, uint deadline)"
        );

    constructor(address _owner) EIP712("ClaimToken", "1") {
        owner = _owner;
    }

    function userClaimToken(
        uint256 uuid,
        uint256 amount,
        address userAddress,
        uint deadline,
...
contract ClaimToken is Pausable, Ownable, ClaimTokenPermit {

    /* ========== EVENTS ========== */

    event UserClaim(uint256 uuid, uint256 amount, address userAddress);
    ...
    function userClaimToken(
        ...
        super.userClaimToken(uuid, amount, userAddress, deadline, v, r, s);
        uint256 balanceOfs = tokenAddress.balanceOf(address(this));
        if (balanceOfs >= amount) {
            tokenAddress.transfer(userAddress, amount);
            emit UserClaim(uuid, amount, userAddress);
        }else{
            revert insufficientBalance(balanceOfs);
        }
    }

    /* ========== VIEWS FUNCTION ========== */

    function _unpackedValue(
        uint256 data
    ) private pure returns (uint256 valueOne, uint128 valueTwo) {
        unchecked {
